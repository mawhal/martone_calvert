...
extractYEAR <- function( data, sheetnames, header=5 ){
...
ptm2012sep  <- lapply( ptm2012, extractYEAR )


# a series of rbinds will group the data together into a meta and a data data.frame
ptm2012bind <- do.call( rbind, ptm2012sep )
# define names for the sheets
sheetnames  <- names(ptm2012sep)
# add sheet names as columns in each list element
ptm2012bindnames <- mapply( cbind, ptm2012bind, "SiteHeightYear"=sheetnames, SIMPLIFY = TRUE)

# separate metadata and data parts of the list
meta <- as_tibble(do.call( rbind, ptm2012bindnames[1:(length(ptm2012bindnames)/2)]))
d    <- as_tibble(do.call( rbind, ptm2012bindnames[(length(ptm2012bindnames)/2)+1:length(ptm2012bindnames)]))
