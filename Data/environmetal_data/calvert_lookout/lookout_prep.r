...
draw <- draw[-c(1:3),]
names(draw)
draw2 <- read_csv( "Data/environmetal_data/calvert_lookout_airtemp_humidity/2020-05-04.1hourSamples.csv" )
draw2 <- draw2[-c(1:3),]
names(draw2)
...
dday <- d %>% 
  ...
  group_by( Year, date ) %>% 
  summarize( temp=mean(temp,na.rm=T),rhmean=mean(rhmean,na.rm=T),
