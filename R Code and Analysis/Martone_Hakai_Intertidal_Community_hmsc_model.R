...
sst <-  pine %>%
  select( Year=year, anom=Anomaly.year, temp=Temp.year )
# merge temperature and the rest of the metadata
M <- left_join( metacomm, sst )
...
## Fixed Effects
XData <- M %>%
  select( site = Site, shore.height = Shore_height_cm, anom )
...
comp.time = matrix(nrow=3, ncol=3)
