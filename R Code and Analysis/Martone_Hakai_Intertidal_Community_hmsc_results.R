...
## load the model
list.files( ModelDir )
mload <- load( paste(ModelDir,list.files( ModelDir ), sep="/") )
m
...
pos.neg$parameter <- factor(c("intercept", "shore.elevation", "temp.anom" ),
                            levels = c("intercept", "shore.elevation", "temp.anom"), 
                            ordered = TRUE)
pos.neg$species <- factor(rep(colnames(postBeta$mean), each = 3), levels = colnames(comm)[order(colSums(comm),decreasing = TRUE)], ordered = TRUE)
...
VP.df <- as.data.frame(VP$vals) %>% 
  mutate(effect = factor(c("elevation","temp.anomaly","transect","year","site"), 
                         levels = rev(c("elevation","temp.anomaly","transect","year","site")), 
                         ordered = TRUE)) %>% 
  gather(key = species, value = variance, -effect) %>% 
  group_by(species) %>% 
  mutate(tempR2 = variance[effect == "temp.anomaly"])
...
R2.df <- data.frame(R2 = round(MF$SR2,2), species = colnames(Y))
...
ggplot(VP.df,aes(y = variance, x = species, fill = effect))+
  ...
  scale_fill_manual(values = c("darkred", "maroon", viridis(4)), name = "")+
