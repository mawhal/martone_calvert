...
## THIS SCRIPT RESAMPLES DATA TO ASSESS HOW WELL PATTERNS ARE CAPTURED WITH SMALLER SAMPLE SIZE
...
## RESAMPLING
# unique transects in each year, each site
set.seed(seed)
kept <- order(runif( 10 ))[1:nkeep]
dkept <- d %>% filter( Quadrat %in% kept )
duse <- dkept

# # all sites
# # time trends in different tidal zones
# (ggzone <- ggplot( duse, aes(x=as.numeric(as.character(Year)),y=Abundance, col=non.alga.flag)) + 
...
# # ggsave( paste0("R Code and Analysis/Figs/algae+invert_means_resamp.pdf", ggzone, "pdf" )


ggplot( duse, aes(x=as.numeric(as.character(Year)),y=Abundance, fill=Zone)) + 
    facet_grid(non.alga.flag~Site, scales="free_y") + 
    geom_smooth( aes(group=Zone), col='black',fill="gray25",alpha=0.15, se=F, show.legend = FALSE ) +
...
ggsave( paste0("R Code and Analysis/Figs/cover_algae+invert_6panel_resamp_",seed,"_keep",nkeep,".svg"), width = 6, height=4 )
