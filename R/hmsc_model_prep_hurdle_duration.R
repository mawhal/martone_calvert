...
XData.raw <- M %>%
  ungroup() %>%
  select( year=Year, duration=duration, shore.height = Shore_height_cm )#, anom.pine.sum.1, anom.pine.win ) #%>%
...
XData <- data.frame( XData.raw$year, XData.raw$duration, XData.raw$shore.height, 
            as.data.frame(poly(XData.raw$shore.height,2)) )
names(XData) <- c("year","duration","elev","elev1","elev2")
...
XFormula = ~ year + duration + elev1 + elev2 +
  elev1:year + elev1:duration#+ elev1:year2 #+ elev2:year1  
...
save(models, modelnames, file="R/output/hmsc_hurdle_duration_specified.Rdata")
