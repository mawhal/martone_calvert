...
# remove  UIDs with no Shore height information (should be just one quadrat position)
d.simple <- d.simple[ !(d.simple$UID %in% remuid), ]
# remove taxa without functional groups
...
ggsave("R/Figs/rank_abundance_hmsc.svg", width = 4, height = 3 ) # note that zeros are not included here
...
# read heatwave duration (using PCA scores from three lighthouses)
duration <- read_csv( "R/output/heatwaveR_duration_surveyyear_pca.csv") %>% 
  mutate(Year = year) %>% select(Year, duration=duration_pca)
plot(duration)
M <- left_join( metacomm, duration )
# M <- metacomm
...
XData.raw <- M %>%
  ungroup() %>%
  select( duration=duration, shore.height = Shore_height_cm )#, anom.pine.sum.1, anom.pine.win ) #%>%
...
XData <- data.frame( XData.raw$duration, XData.raw$shore.height, 
            as.data.frame(poly(XData.raw$shore.height,2)) )
names(XData) <- c("duration","elev","elev1","elev2")
...
XFormula = ~ duration + elev1 + elev2 +
  elev1:duration + elev2:duration #+ elev1:year2 #+ elev2:year1  
...
mprobit <- Hmsc( Y = Ypa, 
           ...
           ranLevels = list(year=rL_year, site=rL_site, transect=rL, quadrat=rL_quad ) )
                             # ty=rL_ty ) ) 
...
save(models, modelnames, file="R/output/hmsc_hurdle_duration2_specified.Rdata")
