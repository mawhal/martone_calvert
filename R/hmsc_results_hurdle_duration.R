...
## load the model
list.files( ModelDir )
model = "model_elevxyearxduration_hurdle_chains_4_thin_100_samples_250.Rdata"
...
for( i in 1:length(models) ){
  VP.df <- as.data.frame(VP[[i]]$vals) %>%
    mutate(effect = factor(c("year","duration","elev1","elev2",
                             "elev1:year","elev1:duration",
                             "site","transect","quadrat"),
                           levels = rev(c("year","duration","elev1","elev2",
                                          "elev1:year","elev1:duration",
                                          "site","transect","quadrat")),
                           ordered = TRUE)) %>%
...
XData_choose <- models[[1]]$XData %>%
  select(year,duration) %>% mutate_all(round,7) %>%  distinct()
...
years <- as.data.frame( models[[1]]$XData %>% select(year,duration) %>% distinct() )
...
names(newXData)
newXData <- newXData %>% 
  select(year,elev,duration,elev1,elev2)
...
ogd$Year 
ogd <- left_join( ogd, read_csv("R/output/heatwaveR_duration_surveyyear_pca.csv") %>% rename( Year = year, duration = duration_pca ) )
ogd$duration
