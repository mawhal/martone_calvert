...
## load the model
list.files( ModelDir )
model = "model_elevxyearxduration2_hurdle_chains_4_thin_100_samples_250.Rdata"
...
for( i in 1:length(models) ){
  VP.df <- as.data.frame(VP[[i]]$vals) %>%
    mutate(effect = factor(c("duration","elev1","elev2",
                             "elev1:duration","elev2:duration",
                             "year","site","transect","quadrat"),
                           levels = rev(c("duration","elev1","elev2",
                                          "elev1:duration","elev2:duration",
                                          "year","site","transect","quadrat")),
                           ordered = TRUE)) %>%
...
XData_choose <- models[[1]]$XData %>%
  select(duration) %>% mutate_all(round,7) %>%  distinct()
XData_choose <- data.frame( year=2012:2019, duration=c(0,XData_choose$duration))
...
years <- as.data.frame( models[[1]]$XData %>% select(duration) %>% distinct() )
...
newXData   <- newDF[,2:(ncol(newDF)-3)]
newDesign  <- data.frame( year = as.factor(newDF$year),site = as.factor(newDF$site), transect = as.factor(newDF$transect),
                          quadrat = as.factor(newDF$quadrat))
names(models[[1]]$XData)
names(newXData)
newXData <- newXData %>% 
  select(elev,duration,elev1,elev2) # year,


## predictions of individual models
predY_pa <- predict(models[[1]], XData = newXData,
                 studyDesign = newDesign,
                 ranLevels = list(year = models[[1]]$rL$year, site = models[[1]]$rL$site, transect = models[[1]]$rL$transect, quadrat = models[[1]]$rL$quadrat), expected = TRUE)
predY_cop <- predict(models[[2]], XData = newXData,
                     studyDesign= newDesign,
                     ranLevels = list(year = models[[1]]$rL$year, site = models[[1]]$rL$site, transect = models[[1]]$rL$transect, quadrat = models[[1]]$rL$quadrat), expected = TRUE) #, transect=rL, year=rL_year
...
ogd$Year 
heatwave <- read_csv("R/output/heatwaveR_duration_surveyyear_pca.csv")  %>% 
  select( -Date )
ogd <- left_join( ogd, heatwave)
ogd$duration

ogd.long <- ogd %>% 
  pivot_longer( cols="Acrosiphonia":"Unknown.crust", 
                names_to = "taxon", values_to = "N" ) %>% 
  select( UID, year, duration, elev = Shore_height_cm, taxon, N )
...
# plot responses
predictions_pa$year <- rep(XData_choose$year, length.out = nrow(predictions_pa))
predictions_pa$yearplot <- as.character(factor(predictions_pa$year, levels = unique(predictions_pa$year), labels = unique(newDF$year)))
predictions_cop$year <- rep(XData_choose$year, length.out = nrow(predictions_cop))
predictions_cop$yearplot <- as.character(factor(predictions_pa$year, levels = unique(predictions_pa$year), labels = unique(newDF$year)))
predictions_abun$year <- rep(XData_choose$year, length.out = nrow(predictions_abun))
predictions_abun$yearplot <- as.character(factor(predictions_pa$year, levels = unique(predictions_pa$year), labels = unique(newDF$year)))
...
Gradient <- constructGradient(models[[1]], focalVariable = "year", non.focalVariables = list("elev1" = list(1),'duration' = list(2)),
                              ngrid = 8) #length(unique(models[[1]]$XData$year1)))

Gradient$XDataNew$year
Gradient$XDataNew$duration <- models[[1]]$XData %>% 
  select(year, duration) %>% distinct() %>% select( duration) %>%  unlist()
...
ggsave("R/Figs/hmsc_sp_trends_duration.svg", height = 5.75*1.5, width = 9*1.5)
#
...
ggplot( producer_bmass_total, aes(x=year,y=median)) + geom_line() +
  geom_line( aes(y=quant_0.25), lty=2) + geom_line( aes(y=quant_0.75),lty=2) +
  ylim(c(0,225))
